...
#### 跨租户用户名隔离
- 修复在某租户创建用户时误报"用户名已存在"的问题（实为其他租户的同名用户）
- 用户名唯一性检查改为按 `用户名 + 租户` 组合验证，不同租户可拥有同名用户
- **修改文件**：`app.py`、`models.py`
- **数据库变更**：`user` 表唯一约束从 `UNIQUE(username)` 改为 `UNIQUE(username, customer_id)`
...
#### 1. 自助注册与试用期
- 新增公开注册页 `/register`，用户可自助填写公司名+管理员账号密码完成注册
- 注册后自动创建租户和管理员账号，试用期30天
- 试用期到期后登录自动拦截并提示联系管理员续期
- **修改文件**：`models.py`、`app.py`、`templates/register.html`（新建）

#### 2. 登录页加公司选择框（解决同名用户冲突）
- 登录页新增公司下拉框，动态列出所有租户
- 登录验证改为按 `公司 + 用户名` 组合查找，不同租户的同名员工（如"张军"）完全独立
- 选择公司时自动更新页面品牌（公司名称）
- 登录页底部新增"免费试用30天"入口链接
- **修改文件**：`app.py`、`templates/login.html`

### 数据库变更
- `tenant_customer` 表新增 `trial_expires_at` 字段（DATETIME，可为空）
- 升级时需执行迁移脚本（见部署说明）
...
#### 1. 新建合同时产品信息自动带入产品管理数据
- 在"新建合同"界面输入产品名称时，模糊搜索下拉列表选中产品后自动填入：产品类型、型号、单位、税率、参考数量、参考单价
- **修改文件**：`app.py`（`/api/products/search` 接口新增返回 `category` 字段）、`templates/contract_form.html`（`selectProduct` 函数新增 `category, taxRate, refQty, refPrice` 参数）

#### 2. 新建合同产品名称搜索无结果时提示去产品管理添加
- 输入产品名称模糊搜索无匹配结果时，下拉框显示"未找到匹配产品 去产品管理添加 ↗"提示链接
- **修改文件**：`templates/contract_form.html`

#### 3. 组织结构管理权限勾选框对齐
- "新建组织"和"编辑组织"弹窗中，权限管理8个勾选项改为 flex 布局，勾选框与文字内容对齐显示
- **修改文件**：`templates/organizations.html`

#### 4. 统计分析显示维度勾选框对齐
- 统计分析页"显示维度"6个勾选项改为 flex 布局，勾选框与文字内容对齐显示
...
#### 1. 租户客户管理新建时权限扩展为8项
- 新建租户客户时权限从6项扩展为8项：增加、删除、修改、查阅、上传、下载、导入EXCEL、导出EXCEL
- **修改文件**：`templates/tenant_management.html`

#### 2. 统计分析页导出Excel权限控制
- 无"导出EXCEL"权限的用户，统计分析页不显示导出按钮，访问导出路由返回权限提示
- 有"导出EXCEL"权限的用户可正常导出
- **修改文件**：`app.py`（`export_statistics`、`statistics`）、`templates/statistics.html`
...
#### 1. 组织权限新增"导入EXCEL"和"导出EXCEL"
- 新建/编辑组织时权限管理从6项扩展为8项：增加、删除、修改、查阅、上传、下载、导入EXCEL、导出EXCEL
- 人员调动后成员自动继承新增权限
- **修改文件**：`templates/organizations.html`
...
- 添加用户前新增全局用户名唯一性校验，重复时返回友好提示而非500错误
- **修改文件**：`app.py`（`new_user`）
...
#### 1. 组织管理新增权限管理
- 新建/编辑组织时可设置权限（增加、删除、修改、查阅、上传、下载）
- 人员调动完成后，成员自动继承所属组织权限
- 新增数据库字段：`organization.permissions`
- **修改文件**：`models.py`、`app.py`（`create_organization`、`edit_organization`、`transfer_user`）、`templates/organizations.html`

#### 2. 用户管理删除权限选项
- 添加/编辑用户时不再显示权限复选框，权限统一由组织管理
- **修改文件**：`templates/user_form.html`

#### 3. 导出Excel合并同一合同编号
- 同一合同编号下的多个产品，合同级字段（合同编号、客户名称、项目名称、合同总价等14列）自动合并单元格
- 导出列顺序：合同编号、客户名称、项目名称、产品名称、型号、单位、数量、单价、合同总价、发票税率、合同类型、业务类型、项目负责人、销售人员、签订日期、状态、已收付款、未收付款、已开票、未开票（共20列）
- **修改文件**：`app.py`（`export_contracts`）

#### 4. 合同列表新增"编辑"按钮
- 操作列新增"编辑"按钮，与"查看"并列显示
- "编辑"进入可操作模式，显示基础信息、产品明细、收付款记录、交付记录、发票记录的完整添加/删除功能
- 新增路由：`GET /contract/<id>/manage`
- **修改文件**：`app.py`、`templates/index.html`

#### 5. 查看模式隐藏编辑功能
- "查看"进入只读模式，隐藏"编辑合同"按钮及收付款、交付、发票的添加表单
- **修改文件**：`templates/contract_detail.html`

#### 6. 发票号重复检测
- 添加发票记录时检测发票号是否已存在，重复时提示"请核对重新输入"并阻止提交
- **修改文件**：`app.py`（`add_invoice`）、`templates/contract_detail.html`

#### 7. 导入Excel修复多产品合并单元格
- 导入时对合同级字段（合同编号、客户名称、项目名称、合同总价、签订日期、合同类型、业务类型、项目负责人、销售人员、状态、发票税率）执行向下填充（ffill），解决Excel合并单元格导致第2行起为空的问题
- **修改文件**：`app.py`（`import_contracts`）

#### 8. 收付款/交付/发票记录支持多文件上传
- 添加记录时文件选择框支持一次选择多个文件，多文件以逗号分隔存储
- 列表显示时每个文件独立显示预览/下载链接
- **修改文件**：`app.py`（`add_payment`、`add_delivery`、`add_invoice`）、`templates/contract_detail.html`

#### 9. 统计导出明细sheet合并同一合同编号
- 统计导出明细数据sheet与合同列表导出一致，同一合同编号的合同级列自动合并单元格
- **修改文件**：`app.py`（`export_statistics`）

#### 10. 编辑合同页面"添加产品"按钮改为蓝色
- "添加产品"按钮由灰色（btn-secondary）改为蓝色（btn-primary）
- **修改文件**：`templates/contract_form.html`
